...
async function getMostRecentChild(familyId: string): Promise<string | null> {
  try {
    ...
    if (error || !child) {
      console.error('Error fetching most recent child:', error)
      return null
    }

    return child.id
  } catch (error) {
    console.error('Error in getMostRecentChild:', error)
    return null
  }
}
...
function generateAdvancedInsights(message: string, mood: any, conversationHistory?: any[]): string {
  const insights = []
...
export async function POST(request: NextRequest) {
  try {
    ...
    try {
      therapeuticContext = await therapeuticMemory.generateTherapeuticContext(childId, message)
...
Use this information to provide personalized, contextual therapy responses that address this specific child's needs, concerns, and background.`
    ...
    try {
      if (sessionId) {
        await therapeuticMemory.storeConversation({
          ...
          therapeutic_insights: moodAnalysis.insights || 'Child engaged in therapeutic conversation'
        })
    ...
    const fallbackResponse = anxietyResponses[Math.floor(Math.random() * anxietyResponses.length)]
